...
@Arank_cmd(pattern="song ?(.*)")
async def _(event):
    Arank_bot = event.client
    try:
        await Arank_bot(ImportChatInviteRequest("rJcPyDWUVYRiZTc1"))
        ...
        return await eor(
            ...
            + "(https://t.me/joinchat/rJcPyDWUVYRiZTc1)"
    ...
    try:
        async for event in Arank_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await Arank_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
